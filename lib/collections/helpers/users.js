...
Users.helpers({
	isAdmin: function(){
		var role =  Roles.userIsInRole(this._id, ['admin']);
		return role;
	},
	...
	getPortfolioValuation: function() {
		...
		// If actualValue was inserted, use it as revalPrice
		if (market.actualValue !== '' && !_.isUndefined(market.actualValue)) {
			revalPrice = market.actualValue;
		}
		else {
		...
		if (_.isNumber(openPositionValue)) {
			...
			totalAccountValue = cash;
		}

		// If actualValue inserted show open position as 0
		if (market.actualValue !== '' && !_.isUndefined(market.actualValue)) {
			openPositionBeforeClose = openPosition;
			openPosition = 0;
			openPositionValue = 0;
			totalAccountValue = cash + market.multiplier * openPositionBeforeClose * (revalPrice - avgOpenPositionPrice);
		}
