...
executeTrade = function(newOrder, newOrderId) {
	...
	if (newOrder.side === 'buy'){
		...
		updateUserAccount(Meteor.userId(), size_traded, 'buy');

		// Update seller account
		updateUserAccount(offer.userId, size_traded, 'sell');
		...
		// Update buyer account
		updateUserAccount(Meteor.userId(), size_traded, 'sell');

		// Update seller account
		updateUserAccount(bid.userId, size_traded, 'buy');
	} 
}
...
updateUserAccount = function(userId, contracts_traded, direction) {
	var portfolio = 						Portfolios.findOne({ userId: userId });
	var statistics = 						Statistics.findOne();

	var open_position = 				portfolio.open_position;
	var new_number_of_trades =	portfolio.number_of_trades + 1;	
	var new_contracts_traded = 	portfolio.contracts_traded + contracts_traded;
	var open_position_value;
	var avg_open_position_price;
	var total_account_value =		portfolio.cash;
	...
	var trades;
	var sum_trades = 				0;
	var sum_trades_price = 	0;

	// Calculate avg_market_price as avg from best bid and best offer
	// if there is no offer or no bid or both, use last trade price.
	if (best_bid && best_offer) {
		avg_mkt_price = (best_bid.price + best_offer.price) / 2;
	} else { 
		avg_mkt_price = statistics.last;
	}
	...
	if (direction === 'buy') {
		open_position += contracts_traded;
	} else {
		open_position -= contracts_traded;
	}
	...
	if (open_position === 0 ) {
		...
		trades = Prices.find({ userId: userId, side: 'buy' });
	} 
	else  {
		trades = Prices.find({ userId: userId, side: 'sell' });
	}
		
	trades.forEach(function(trade){
		sum_trades_price 	+= trade.price * trade.size;
		sum_trades 				+= trade.size;
	});

		avg_open_position_price = Math.round(sum_trades_price / sum_trades * 10	)/10;

	Portfolios.update(portfolio, { $set: {
		number_of_trades: 				new_number_of_trades,
		contracts_traded: 				new_contracts_traded,
		open_position: 						open_position,
		avgOpenPositionPrice:  		avg_open_position_price,
	}});
}
