...
Prices.allow({
	...
	remove: function(userId, order) { 
		...
		return ownsOrder(userId, order) || user.isAdmin(); 
	},
});
...
Prices.helpers({
	removeBid: function(){
		...
		// if bid consists of single price
...
Meteor.methods({
	...
	removeTrades: function(){
		if (Meteor.user().isAdmin()) {
			...
			// Clear trade dependent market fields
...
updateStatistics = function(contracts_traded, price) {
	...
	var total_contracts_traded = 		market.total_contracts_traded + contracts_traded;
	var number_of_trades =					market.number_of_trades + 1;
	var open =											market.open || price;
	var high =	 										Math.max(market.high, price) || price;
	var low =												Math.min(market.low, price) || price;
	var last = 											price;

	Markets.update(market, { $set: {
...
validateOrder = function(order) {
	...
	if ( _.isNumber(order.size) ) {
		if (orderToBig(order) ) {
			errors.orderToBig = "Order size is to big. Actual position size limit is 250 contracts."
		}
	}
...
orderToBig = function(order) {
	var market = Markets.findOne({});
	var user = Meteor.user()

	if (user.openPosition() + order.size > market.maxPosition) {
		return true;
	}
