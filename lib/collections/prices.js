...
updatePortfolio = function(){
		...
		console.log('buy trades collection, ', buyTrades.fetch());
		console.log('contracts Bought ', contractsBought);
		...
		console.log('avgOpenPositionPrice ', avgOpenPositionPrice);
		...
		console.log('updating newValues', newValues);
...
avgCollectionPrice = function(collection, contractsNumber) {
	zwrot = _.reduce(collection, function(m, item){
						console.log('m, item.size, item.price ', m, item.size, item.price);
						return m + item.size * item.price;
					}) / contractsNumber;
